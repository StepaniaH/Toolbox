...
[`$develop-toolbox-tool`](./.agents/skills/develop-toolbox-tool/SKILL.md)，自行推导
Brief 与安全默认值，并补齐平台接入、双语文档、隐私与测试。全部实现以聚焦提交落在本地
`dev` 分支；对照 `main` 的审核、合并、推送与发布 tag 是独立的维护者步骤，见
