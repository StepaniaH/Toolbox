...
| Settings | `/settings/` | 语言、明暗模式、配色族与首页个性化 | React + TS + Vite |

## 设计原则

- **隐私优先** — 核心计算留在客户端；无自有后端、无追踪、无 Cookie；外部数据访问必须可见、可选并有本地 fallback。
- **三语** — 每个稳定工具均提供简体中文 / 繁體中文 / English 界面。
- **统一外观** — Catppuccin、Gruvbox、Solarized 三套配色各含深浅两种模式，全部在设置页切换；语言与外观没有第二套入口。
- **MIT 开源** — 所有工具均采用 MIT 许可证。
- **静态部署** — 构建为纯静态文件，统一域名下路径路由，经 Cloudflare Pages 发布。
