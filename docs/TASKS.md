...
- [x] 在自动识别首页之外增加 PDF、压缩包等手动文件族 Tab，两条入口共享相同能力状态与隐私说明。
- [x] 修复文本工作台格式栏与双栏编辑器对齐；默认不创建示例文档，并提供无装饰性符号的空状态与明确新建入口。
- [x] 首阶段开放 PDF 基础检查与 ZIP 中央目录清单/受限本地提取，恶意路径、加密、未知压缩和展开体积必须显式拒绝。
- [x] 收尾修正 XLSX 等 ZIP 容器格式误路由、PDF/ZIP 操作栏完成度、文本编辑器顶部对齐，并把 PDF/压缩包纳入知识库分类与对比。
- [x] 统一文件首页、图片、GIF、文本、PDF 与压缩包 Tab 的文件选择按钮、焦点与无障碍契约。
- [x] 集成审核补强 Markdown 危险链接、SVG 嵌套数据源、ZIP 重名/展开上限和畸形本地设置恢复。
- [x] 作为 `v0.2.5` 发布候选完成双语品牌副标题、stable 导航展示和生产部署清单接入。
- [x] 作为 `v0.2.6` 增量优化浅色卡片层级，开放图片队列信息与批量旋转/翻转，并验证变换始终重新编码。
