...
### P0.1 · RateLens 外部汇率请求改为主动选择 `⏳ 待开始`
...
- [ ] 首次加载不产生业务第三方网络请求。
- [ ] 默认参考汇率或手动输入可完成所有计算。
- [ ] “获取实时汇率”由用户点击触发，并显示简短隐私说明。
- [ ] fetcher 有超时、fallback 和无真实网络的测试。
- [ ] README/页面隐私表述与实际行为一致。
- [ ] 建立外部 URL allowlist，CI 能发现新端点。

### P0.2 · 加固手动部署脚本 `⏳ 待开始`

**发现**：脚本当前尝试 `checkout main` 并忽略失败；在脏工作区或切换失败时可能继续从错误分支部署。脚本还引用了缺失的 `.env.example`，并含不应进入公开默认值的具体端口。

**目标**：部署脚本只验证，不替用户切分支；只允许干净且已同步的 `main`。

**验收**：

- [ ] 非 `main`、detached HEAD、dirty worktree 立即失败。
- [ ] 更新只允许 `git pull --ff-only`，不自动 merge/rebase。
- [ ] 端口、主机、用户和路径全部来自 gitignored env；公开模板只含通用占位符。
- [ ] 补 `deploy/.env.example`，脚本对缺失字段给出不回显值的错误。
- [ ] shellcheck 或最小脚本测试通过。
...
- [x] 将本仓库未来提交邮箱配置为 GitHub noreply，并在提交前验证。
- [ ] 决定是否改写 v0.1 的既有提交邮箱历史。
- [ ] CI 增加 secret scan；扫描结果不得把疑似 secret 原文打印到日志。

**注意**：既有 5 个提交使用同一个非-noreply 邮箱。改写已发布 `main` / `v0.1` 会改变 commit SHA 并需要 force push，必须由维护者单独批准；当前任务不得自动执行。
