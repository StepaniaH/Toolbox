...
- [ ] 在自动识别首页之外增加 PDF、压缩包等手动文件族 Tab，两条入口共享相同能力状态与隐私说明。
- [ ] 修复文本工作台格式栏与双栏编辑器对齐；默认不创建示例文档，并提供无装饰性符号的空状态与明确新建入口。
- [ ] 首阶段开放 PDF 基础检查与 ZIP 中央目录清单/受限本地提取，恶意路径、加密、未知压缩和展开体积必须显式拒绝。
