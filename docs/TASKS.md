...
- [x] 文件首页完成本地轻量识别、能力分级与显式分流；图片、GIF、文本、表格数据、PDF 和 ZIP 保持隔离工作台与各自预算。
- [x] PDF 轻量检查与防路径穿越/重名/压缩炸弹的 ZIP 清单和选择性提取已经开放。
- [x] HEIC/HEIF 获得 64 MB 上限的按需本地解码；CSV/TSV/XLSX 获得独立值转换、公式注入防护、宏/公式不执行和表格资源上限。
- [x] 共享选择器、浅色层级与页面结构使用边界、列表和表格，持续降低同层级 Card 密度。
- [ ] 分阶段实现裁剪/拼接等图片编辑、GIF 专项、PDF 页面、JSON/YAML/XML 数据和其他压缩格式；每阶段独立测试、
  文档和本地提交，重型解析器需先通过依赖、内存与安全评估。

### P1.6 · CryptoLab 本地密码学工具 `✅ 已完成`

- [x] 已在独立 `newdev/crypto-lab` 完成、审核并按维护者授权合并到 `dev`；候选继续保持 hidden，开发分支已删除。
