...
| Monitor Choice | `/monitor-choice/` | Vanilla JS + Vite + Canvas | 显示器参数实验室 | 16 |
| SaneUnits | `/sane-units/` | React + TypeScript + Vite + Plain CSS | 单位换算与现实估算 | 18 |

测试数量只用于说明覆盖现状，不作为质量本身的替代指标。5 个应用当前有 945 条测试，另有 5 条 app manifest 和 11 条 theme 契约测试；`pnpm test` 共运行 961 条。`v0.1` 发布时为 910 条。
...
| `pnpm test` | 通过 | 961 tests；数量不等同于覆盖率 |
