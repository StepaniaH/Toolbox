...
| FormTran | `/image-converter/` | React + TypeScript + Vite + Plain CSS | 浏览器本地文件、图片、表格、PDF 与 ZIP 工作台 | 61 |
| CryptoLab（hidden） | `/crypto-lab/` | React + TypeScript + Vite + Tailwind | 本地密码学、公钥二维码安全分享与学习工具 | 109 |

测试数量只用于说明覆盖现状，不作为质量本身的替代指标。7 个应用当前有 1,120 条测试，另有 5 条 app manifest 和 11 条 theme 契约测试；完整工作区共运行 1,136 条。CryptoLab 已进入 `dev` 但仍为 hidden，不会出现在稳定导航或部署清单。`v0.1` 发布时为 910 条。
...
| `pnpm test` | 通过 | 1,136 tests；数量不等同于覆盖率 |
| `pnpm test:browser` | 通过 | 七个应用均有生产态回归；FormTran 额外覆盖真实 HEIC、XLSX 往返、多页 PDF 合并与共享结果 ZIP，CryptoLab 覆盖真实 Secure Share 二维码链路与 Tab 无纵向溢出 |
