...
>
> 阅读顺序：本文 → [INDEX.md](./INDEX.md) → [PLAN.md](./PLAN.md) → [TASKS.md](./TASKS.md) → 目标工具 README / 局部 AGENTS。
...
4. 当前 Homepage 与 Monitor Choice 未进入根质量命令；涉及它们时必须额外做静态/浏览器 smoke，不能用根命令通过代替。
5. warning 必须记录并避免新增；不能长期用宽松退出码掩盖缺口。
