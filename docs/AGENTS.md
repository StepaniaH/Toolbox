...
- 设置页语言下拉的选项始终使用该语言的自称（`中文（简体）`、`English`），不随当前界面语言翻译。
- Settings 是偏好入口而非独立工具，manifest 中 `presentation.card = false`，不进入首页卡片与导航工具菜单。
...
视觉任务还需按 DESIGN_SYSTEM 的发布前设计检查执行。新增工具必须显式调用仓库内
`$develop-toolbox-tool` skill，并按 [NEW_TOOL.md](./NEW_TOOL.md) 执行。
