...
1. UI 明确说明数据用途和第三方服务；自动请求的例外必须在首屏持续可见。
2. 默认由用户主动操作触发。只有“当前数据本身就是核心功能”且维护者在 app brief 中明确批准时，才允许页面加载自动请求。
3. 不发送工具输入、持久化数据或可识别用户的信息，除非用户明确知道且任务本身必需。
4. 提供手动值、经时效说明的参考值或离线模式；不允许用未标日期的硬编码值伪装实时数据。
...
- [ ] 计算、错误恢复和主流程测试通过。
- [ ] 无未批准的自动第三方访问；主动请求或获批的自动实时请求均已披露、最小化并有失败恢复。
