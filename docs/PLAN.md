...
1. **Settings 应用**（`apps/settings`，`/settings/`）：Appearance（主题模式、主题族、
   语言）与 Homepage（展示哪些工具、每行/总数密度、展示顺序）两节起步。全局 NavBar 的
   快捷语言/主题按钮保留，设置页只承载扩展项；AGENTS 设计偏好中“语言与主题只由全局导航
   提供”相应放宽为“快捷切换只由全局导航提供”。
...
应用可以拥有更丰富的本地偏好模式（例如“跟随系统”），但模式标识只能保存在应用私有
命名空间（`toolbox.<app-id>.*`）；共享键在任何时刻都只保存解析后的契约值。读取到非法或
遗留值时按契约回退，不扩大故障半径。
