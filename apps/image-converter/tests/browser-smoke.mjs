...
try {
  ...
  assert.equal(await page.locator('.knowledge-comparison table').isVisible(), true)
  await page.locator('.knowledge-categories button').filter({ hasText: 'PDF' }).click()
  assert.equal(await page.locator('.knowledge-topic article').count(), 4)
  assert.match(await page.locator('.knowledge-comparison').textContent(), /PDF capability levels|PDF 能力层级/)
  await page.locator('.knowledge-categories button').filter({ hasText: /Archives|压缩包/ }).click()
  assert.equal(await page.locator('.knowledge-topic article').count(), 4)
  assert.match(await page.locator('.knowledge-comparison').textContent(), /Archive support scope|压缩包支持范围/)
  ...
  const editorHeaders = await page.locator('.editor-card header').evaluateAll((headers) => headers.map((header) => header.getBoundingClientRect()))
  const editorAreas = await page.locator('.editor-card textarea').evaluateAll((areas) => areas.map((area) => area.getBoundingClientRect()))
  assert.equal(Math.abs(editorHeaders[0].height - editorHeaders[1].height) < 1, true)
