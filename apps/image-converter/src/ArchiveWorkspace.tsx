...
export function ArchiveWorkspace({ hidden, incoming }: { hidden?: boolean; incoming?: Incoming }) {
  ...
  const open = async (next: File | undefined) => {
    if (!next) return;
    setFile(next); setDirectory(null); setSelected(new Set()); setError(null); setBusy(true);
  ...
  const clear = () => { setFile(null); setDirectory(null); setSelected(new Set()); setError(null); };
  ...
  return <section className="family-page" role="tabpanel" id="panel-archive" aria-labelledby="tab-archive" hidden={hidden}>
    <header className="family-header"><div><span className="eyebrow">ARCHIVE WORKSPACE</span><h2>{t("archive.title")}</h2><p>{t("archive.intro")}</p></div><div className="section-actions"><label className="button primary">{t("archive.open")}<input type="file" accept=".zip,application/zip" onChange={onInput}/></label>{file && <button className="text-button" type="button" onClick={clear}>{t("archive.clear")}</button>}</div></header>
