...
export function PdfWorkspace({ hidden, incoming }: { hidden?: boolean; incoming?: Incoming }) {
  ...
  return <section className="family-page" role="tabpanel" id="panel-pdf" aria-labelledby="tab-pdf" hidden={hidden}>
    <header className="family-header"><div><span className="eyebrow">PDF WORKSPACE</span><h2>{t("pdf.title")}</h2><p>{t("pdf.intro")}</p></div><div className="family-actions"><FilePicker label={t("pdf.open")} accept=".pdf,application/pdf" onChange={onInput}/>{file && <button className="button secondary" type="button" onClick={clear}>{t("pdf.clear")}</button>}</div></header>
