...
export function PdfWorkspace({ hidden, incoming }: { hidden?: boolean; incoming?: Incoming }) {
  ...
  return <section className="family-page" role="tabpanel" id="panel-pdf" aria-labelledby="tab-pdf" hidden={hidden}>
    <header className="family-header"><div><span className="eyebrow">PDF WORKSPACE</span><h2>{t("pdf.title")}</h2><p>{t("pdf.intro")}</p></div><div className="family-actions"><label className="family-upload"><span aria-hidden="true">＋</span><strong>{t("pdf.open")}</strong><input type="file" accept=".pdf,application/pdf" onChange={onInput}/></label>{file && <button className="family-clear" type="button" onClick={clear}><span aria-hidden="true">×</span>{t("pdf.clear")}</button>}</div></header>
