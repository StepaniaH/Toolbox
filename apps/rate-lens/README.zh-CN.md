...
页面首次加载时会从 `open.er-api.com` 请求 USD/CNY 汇率，失败后回退到 jsDelivr 上的公开汇率数据。请求不会包含计算表单内容，但第三方会收到常规网络元数据（如 IP、请求头和访问时间）。离线时使用内置参考值，也可随时手动输入汇率。

为了与 Toolbox 的严格隐私目标一致，仓库已将“改为用户主动获取、加载前披露第三方、默认纯本地”列为 P0 任务，见 [`../../docs/TASKS.md`](../../docs/TASKS.md)。
