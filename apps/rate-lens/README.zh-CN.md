...
页面默认使用内置参考汇率，不会自动访问第三方。只有用户主动点击“获取实时汇率”时，才会从 `open.er-api.com` 请求 USD/CNY 汇率，失败后回退到 jsDelivr 上的公开汇率数据。请求不会包含计算表单内容，但第三方会收到常规网络元数据（如 IP、请求头和访问时间）。也可以始终使用本地参考值或手动输入汇率。
