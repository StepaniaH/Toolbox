...
- **安全分享**：生成收件人 RSA 密钥对，把短消息加密成带版本的 `CL1` 数据包与二维码；收件人可上传二维码图片/数据包，再用匹配私钥解密。
...
- **RSA 工作台**：生成 2048/4096 位 RSA 密钥对、PEM 导入导出、RSA-OAEP 加解密、RSA-PSS 签名与验证。
...
1. 收件人生成 2048 位 RSA-OAEP/SHA-256 密钥对，只把公钥交给发送方。
...
- 用户内容只保留在当前页面内存，刷新即清除；CryptoLab 不持久化私有业务状态。
...
- 安全分享不验证收件人身份，也不提供密钥托管、撤销、前向保密或受感染浏览器/设备防护；高价值或长期密钥应使用经过审计的密钥管理方案。
