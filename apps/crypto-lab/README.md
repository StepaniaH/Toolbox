...
- **Secure Share**: Generate a recipient RSA key pair, encrypt a short message into a versioned `CL1` packet, create/download a QR code, then import the QR image or packet and decrypt it with the matching private key.
...
- **RSA Workbench**: Generate 2048/4096-bit RSA key pairs, export/import PEM, encrypt/decrypt with RSA-OAEP, sign/verify with RSA-PSS.
...
1. The recipient generates a 2048-bit RSA-OAEP/SHA-256 key pair and shares only the public key.
...
- User content is kept only in current page memory. Refreshing clears it; CryptoLab does not persist private app state.
...
- Secure Share does not establish the recipient's identity or provide key custody, revocation, forward secrecy, or protection from a compromised browser/device. Use audited key-management systems for high-value or long-lived keys.
